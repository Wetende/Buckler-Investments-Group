...
const TourDetail = () => {
  const { id } = useParams()
  const { data, isLoading, isError, error } = useTour(id)
  const { data: avail } = useTourAvailability(id)

  if (isLoading) return <div className="skeleton-loader">Loading...</div>
  if (isError) return <div className="text-red-500">{error?.message || 'Failed to load tour'}</div>
        ...
        <Container>
          <Row className="mb-10">
            <Col>
              ...
              <div className="text-[#777]">Duration: {data?.duration}</div>
            </Col>
          </Row>
          {gallery.length > 0 && (
            <Row className="mb-10">
