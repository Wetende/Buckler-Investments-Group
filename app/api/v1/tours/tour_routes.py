...
async def create_or_update_tour(
    ...
    use_case: CreateTourUseCase = Depends(Provide[AppContainer.create_tour_use_case]),
):
...
async def delete_tour(
    tour_id: int,
    use_case: DeleteTourUseCase = Depends(Provide[AppContainer.delete_tour_use_case]),
):
...
async def update_tour_availability(
    ...
    return {"ok": True, "message": "Availability updated", "tour_id": tour_id}

@router.post("/{tour_id}/pricing", response_model=dict)
async def update_tour_pricing(
    ...
    return {"ok": True, "message": "Pricing updated", "tour_id": tour_id}
