"""Admin notification template & provider config management."""
...
from models.notification_template import NotificationTemplate, Channel
from models.provider_config import ProviderConfig, ProviderType
...
router = APIRouter(prefix="/api/v1/admin/notifications", tags=["Notifications (Admin)"])
...
async def create_or_update_template(
    payload: NotificationTemplateCU,
    session: AsyncSession = Depends(get_async_session),
    _: User = Depends(require_admin),
):
    if payload.id == 0:
